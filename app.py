...
st.sidebar.caption("Ajusta esto según la configuración de tu liga (Ej: 10 titulares).")
...
# ==========================================
# 1. THE WEEKLY GRID (LÓGICA CORREGIDA)
# ==========================================
st.header(f"📅 Planificación Semanal (Límite: {limit_slots} jugadores)")
...
if mi_matchup:
    with st.spinner("Calculando Matrix de Calendario..."):
        ...
        for dia in dias_semana:
            ...
            # 2. Aplicamos el CAP (Límite de Titulares)
            # Si tengo 12, solo puedo usar 10. Si tengo 8, uso 8.
            usados_yo = min(disponibles_yo, limit_slots)
            usados_rival = min(disponibles_rival, limit_slots)
            
            # Texto para la tabla
            txt_yo = f"{usados_yo}"
            if disponibles_yo > limit_slots: txt_yo = f"{usados_yo} (de {disponibles_yo})" # Aviso de Overbooking
        ...
        if diff_total < 0:
            st.error(f"⚠️ ALERTA DE VOLUMEN: Proyectas jugar {abs(diff_total)} partidos MENOS que tu rival esta semana.")
...
st.caption("🚀 Fantasy GM Architect v3.2 | Logic Perfected")